...
(function(root){
	...
	var Isobar = (function(global){
		...
		var Isobar = function(context, samples, counterValues, colorLevels, opacity){
			...
			this.radius = 100; // 网格点插值的半径
			this.limitePointCount = 100; // 网格点插值的最大计算数
			this.row = 10; // 行线数
			this.col = 10; // 列线数
			this.opacity = opacity || 1; // 不透明度,默认为1
	...
	var createGrid = function(samples,row,col,width, height,counterValues,radius,limitePointCount) {
		var data = new Array();
		var rowInterval = height / (row - 1);
		var colInterval = width / (col - 1);
		for (var i = 0; i < row; i++) {
			data[i] = new Array();
			for (var j = 0; j < col; j++) {
				var nPointCount = 0,
	...
	var getColor = function(v, colorLevels) {
		for(var i = 0, len = colorLevels.length; i < len; i++){
			...
			if( v > colorLevel.min && v <= colorLevel.max) {
	...
	var fillRegion = function(region, context, color) {
		...
		//绘制等值线
	Isobar.prototype.drawlines = function(){

		var row = this.row,
			col = this.col;
		var data = createGrid(this.samples,
							  row,col,
							  this.width, 
							  this.height,
							  this.counterValues,
							  this.radius,
							  this.limitePointCount);
		var X = new Array(5),
			Y = new Array(5),
			V = new Array(5);
		...
		for(var i = 0; i < row - 1; i++) {
			for(var j = 0; j < col - 1; j++) {
				//设置网格四个角坐标和属性值
				X[0] = i; Y[0] = j; V[0] = data[i][j];   
				X[1] = i + 1; Y[1] = j; V[1] = data[i+1][j];    
				X[2] = i + 1; Y[2] = j + 1; V[2] = data[i+1][j+1];    
				X[3] = i; Y[3] = j + 1; V[3] = data[i][j+1]; 
				//网格中心点坐标和属性值
				X[4] = 0.5 * (X[0] + X[1]); 
				Y[4] = 0.5 * (Y[1] + Y[2]); 
				...
				for(var n = 0; n < 4; n++) {
					...
					for(var line = 0, contourData = this.counterValues; line < contourData.length; line++) {
